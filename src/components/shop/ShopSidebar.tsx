...
import { Filter, Package, ChevronUp, ChevronDown, Settings } from 'lucide-react';
...
export const ShopSidebar: React.FC<ShopSidebarProps> = ({
  ...
  return (
    <div className="hidden lg:block lg:col-span-1">
      <div className="bg-surface p-6 shadow-lg rounded-lg sticky top-32">
        <h3 className="font-semibold text-Color-Netural-Black mb-6 flex items-center">
        ...
        <div className="mb-8">
          ...
          {expandedSections.availability && (
            <div className="space-y-3">
              <label className="flex items-center cursor-pointer hover:bg-Color-Netural-White p-3 rounded-lg transition-colors duration-200">
                <input
                  type="checkbox"
                  checked={showInStockOnly}
                  onChange={(e) => onInStockFilterChange?.(e.target.checked)}
                  className="mr-3 w-4 h-4 text-green-600 border-Color-Light-300 rounded focus:ring-green-500"
                />
          ...
          {expandedSections.categories && (
            <div className="space-y-3">
              {categories.map(category => (
                <label
                  key={category.value}
                  className="flex items-center justify-between cursor-pointer hover:bg-Color-Netural-White p-2 rounded transition-colors duration-200"
                >
                  <div className="flex items-center">
                    <input
                      ...
                      className="mr-3 w-4 h-4 text-Color-Light-300 border-Color-Light-300 rounded focus:ring-Color-Light-300"
                    />
                    <span className="text-Color-Netural-Black font-medium">{category.label}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="text-Color-Champagne-Gold text-sm">({category.count})</span>
                    {category.value === 'Ring' && (
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                        ...
                        className="p-1 hover:bg-Color-Light-300/20 rounded-full transition-colors duration-200"
                        title="Customize Ring"
                      >
                        <Settings className="h-4 w-4 text-Color-Light-300" />
                      </button>
                    )}
                  </div>
                </label>
          ...
          {expandedSections.diamondShapes && (
            <div className="space-y-3">
              {diamondShapes.map(shape => (
                <button
                  key={shape.value}
                  className="w-full flex items-center justify-between p-2 hover:bg-Color-Netural-White rounded transition-colors duration-200 text-left"
                >
                  <div className="flex items-center">
                    <img
                      src={shape.image}
                    ...
                    />
                    <span className="text-Color-Netural-Black font-medium">{shape.label}</span>
                  </div>
                  <span className="text-Color-Champagne-Gold text-sm">({shape.count})</span>
                </button>
              ))}
            </div>
          )}
            ...
            <div className="space-y-3">
              {goldColors.map(color => (
                ...
                >
                  ...
                  <span className="text-Color-Champagne-Gold text-sm">({color.count})</span>
                </label>
              ))}
            </div>
          )}
                ...
                >
                  <div className="flex items-center">
                    <input
                      ...
                      className="mr-3 w-4 h-4 text-Color-Light-300 border-Color-Light-300 rounded focus:ring-Color-Light-300"
                    />
                    <span className="text-Color-Netural-Black font-medium">{size.value}</span>
                  </div>
                  <span className="text-Color-Champagne-Gold text-sm">({size.count})</span>
                </label>
              ))}
            </div>
          )}
        ...
        {/* Filter Summary */}
        <div className="bg-Color-Netural-White p-4 rounded-lg">
          <h4 className="font-medium text-Color-Netural-Black mb-3">Active Filters</h4>
          <div className="space-y-2 text-sm">
            <div className="flex justify-between">
              <span className="text-Color-Netural-Black">Total products:</span>
              <span className="font-medium">{products.length}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-Color-Netural-Black">Available:</span>
              <span className="font-medium text-green-600">
                {products.filter(p => p.availableForSale).length}
              </span>
            </div>
            {selectedGoldColors.length > 0 && (
              <div className="flex justify-between">
                <span className="text-Color-Netural-Black">Gold colors:</span>
                <span className="font-medium text-Color-Light-300">
                  {selectedGoldColors.length} selected
                </span>
              </div>
            )}
            {selectedSizes.length > 0 && (
              <div className="flex justify-between">
                <span className="text-Color-Netural-Black">Sizes:</span>
