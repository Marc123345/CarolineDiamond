...
export const BestsellersPage: React.FC<BestsellersPageProps> = ({ onNavigate }) => {
  const { products, loading, error, usingFallback } = useShopifyProducts('tag:bestseller OR tag:engagement-ring OR tag:solitaire', 'BEST_SELLING', false);
    ...
    <div className="bg-Color-Netural-White min-h-screen">
      ...
      <section className="luxury-section luxury-container">
          ...
          <>
            <div className="flex items-center justify-between mb-8">
              <div className="flex items-center text-Color-Netural-Black">
                <Star className="h-5 w-5 mr-2 fill-Color-Light-300 text-Color-Light-300" />
                <span className="typography-body font-semibold">
                  {products.length} {products.length === 1 ? 'bestseller' : 'bestsellers'}
                </span>
              </div>
            </div>
        ...
        {!loading && !error && products.length === 0 && (
          <div className="text-center py-12">
            <Star className="h-16 w-16 text-Color-Champagne-Gold mx-auto mb-4" />
            <h3 className="typography-h4 text-Color-Netural-Black mb-2">No bestsellers yet</h3>
            <p className="typography-body text-Color-Champagne-Gold mb-6 max-w-md mx-auto">
              Check back soon for our most popular pieces
            </p>
          </div>
        )}
      </section>
